...
update `svsys_config_i18ns` set `value`=replace(`value`,'AmazeUI','default'),`value`=replace(`default_value`,'AmazeUI','default') WHERE `default_value` LIKE '%AmazeUI%' OR `value` LIKE '%AmazeUI%';

--
--	地区数据修复
--
UPDATE `svoms_region_i18ns` SET `name` = REPLACE( `name` , '?', '' ) WHERE `name` LIKE '%?';
UPDATE `svoms_region_i18ns` SET `name` = REPLACE( `name` , '	', '' ) WHERE `name` LIKE '%	';
UPDATE `svoms_region_i18ns` SET `name`=TRIM(`name`) WHERE `name` LIKE '% ';

--
--	网站设置数据修复
--
UPDATE `svsys_config_i18ns` SET `default_value` = TRIM( `default_value` ) ,`value` = TRIM( `value` ) ,`options` = TRIM( `options` ) ,`description` = TRIM( `options` );