		...
		<div>			
			<div class="listtable_div_top  am-panel-body">
				<div class="am-panel-bd am-g">
					...
					<?php if($link['Link']['status']==1){?><span  class="am-icon-check am-yes" style="cursor:pointer;"></span><?php }?>
					<?php if($link['Link']['status']==0){?><span  class="am-icon-close  am-no"style="cursor:pointer;" ></span><?php }?> &nbsp;</div>
					...
					<div class="am-u-lg-2 am-u-md-3 am-u-sm-4  am-action" >
						<?php
							if($svshow->operator_privilege("links_edit")){ ?>
							  <a class="am-btn am-btn-default am-btn-xs  am-seevia-btn-edit" href="<?php echo $html->url('/links/view/'.$link['Link']['id']); ?>">
                        <span class="am-icon-pencil-square-o"></span> <?php echo $ld['edit']; ?>
                    </a> <?php	}?>
							<?php	if($svshow->operator_privilege("links_remove")){?>
						 	<a class="am-btn am-btn-default am-btn-xs am-text-danger  am-seevia-btn-delete" href="javascript:;" onclick="list_delete_submit(admin_webroot+'links/remove/<?php echo $link['Link']['id'] ?>')">
