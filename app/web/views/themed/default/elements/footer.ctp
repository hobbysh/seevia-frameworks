...
<?php if(!empty($links)){?>
<div class="am-g  ">
<div class="am-g-fixed am-g" style="margin:1.5rem 0rem;width:100%;">
  <ul  style="width:100%;" class="am-gallery am-avg-sm-6  am-avg-md-9 am-avg-lg-14 am-gallery-default "data-am-widget="gallery" data-am-gallery="{ pureview: false }" >
  <?php foreach($links as $k=>$v){?>
    <li  class="am-thumbnail   am-icon-sm" style="border:0;width:100px;height:100px;margin-left:5px;">
	    <?php if($v['LinkI18n']['img01']==""){ ?>
			 	  <div style="margin-top:90%;max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"> <?php echo $svshow->link($v['LinkI18n']['name'],$v['LinkI18n']['url'],array('target'=>$v['Link']['target'])); ?></div>
			  <?php  }else{  ?>
		 <?php echo 	$svshow->seo_link(array('type'=>'IMG','class'=>"dd",'url'=>$v['LinkI18n']['url'],'img'=>$v['LinkI18n']['img01'],'name'=>$v['LinkI18n']['name'],'sub_name'=>$v['LinkI18n']['name'],'target'=>$v['Link']['target']));?>
		  <?php }?>
 </li>
  <?php }?>
  </ul>  
