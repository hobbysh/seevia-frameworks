 ...
 */
function check_dirs_priv($checking_dirs,$_LANG=array())
{
    ...
    {
        if (!file_exists(dirname(ROOT_PATH).$dir))
        {
            $msgs['result'] = 'ERROR';
        ...
        if (file_mode_info(dirname(ROOT_PATH).$dir) < 2)
        {
            $msgs['result'] = 'ERROR';
...
function check_rename_priv()
{
    ...
    /* 获取images目录下图片目录 */
    $folder = opendir(dirname(ROOT_PATH) . '/media');
    while ($dir = readdir($folder))
    {
        if (is_dir(dirname(ROOT_PATH) . '/media' . $dir) && preg_match('/^[0-9]{6}$/', $dir))
        {
            $dir_list[] = '/media' . $dir;
        }
    }
