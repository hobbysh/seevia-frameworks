...
class DBManager
{
    ...
    function __construct($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $sprefix = 'ecs_', $tprefix = 'ecs_', $log_path = '', $auto_match = false, $ignored_errors = array()){
    	  $db = new cls_mysql($dbhost, $dbuser, $dbpw, $dbname,$charset);
    ...
    function create_database($db_host, $db_port, $db_user, $db_pass, $db_name){
	    ...
	    if (mysql_select_db($db_name, $conn) === false){
	        $sql = $mysql_version >= '4.1' ? "CREATE DATABASE $db_name DEFAULT CHARACTER SET " . $this->db_charset : "CREATE DATABASE $db_name";
	...
	function keep_right_conn($conn, $mysql_version='')
	{
		...
		if ($mysql_version >= '4.1'){
	        mysql_query('SET character_set_connection=' . $this->db_charset . ', character_set_results=' . $this->db_charset . ', character_set_client=binary', $conn);
